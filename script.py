import cv2
allfaces = {1: "Angelina", 2: "Bob"}

def gen_data(img, id, img_id):
    cv2.imwrite("data/user." + str(id) + "." + str(img_id) + ".jpg", img)

def draw_bounds(img, classifier, scale, neighbors, color, text, clf):
    ...
    for (x, y, w, h) in features:
        ...
        if id in allfaces.keys():
            cv2.putText(img, allfaces[id], (x, y - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1, cv2.LINE_AA)
...
def recognize(img, clf, faceCascade):
    ...
    coords = draw_bounds(img, faceCascade, 1.1, 10, color["blue"], "Face", clf)
    return img
